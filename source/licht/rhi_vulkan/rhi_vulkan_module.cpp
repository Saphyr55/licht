...
void RHIVulkanModule::tick() {
    ...
    VkQueue graphics_queue = vulkan_query_queue(context, VulkanQueueFamilyType::Graphics);
    VkQueue present_queue = vulkan_query_queue(context, VulkanQueueFamilyType::Present);
    ...
    VkResult acquire_next_image_result = VulkanAPI::lvkAcquireNextImageKHR(context.device, context_->swapchain->get_handle(), UINT64_MAX, context_->image_available_semaphores[context_->current_frame], VK_NULL_HANDLE, &image_index);
    ...
    vulkan_render_pass_begin(context_, context_->command_buffers[context_->current_frame], image_index);
    {
        ...
        viewport.x = 0.0f;
        viewport.y = 0.0f;
        viewport.width = static_cast<float32>(context_->swapchain->get_extent().width);
        viewport.height = static_cast<float32>(context_->swapchain->get_extent().height);
        ...
        VkRect2D scissor{};
        scissor.offset = {0, 0};
        scissor.extent = context_->swapchain->get_extent();
    ...
    LICHT_VULKAN_CHECK(VulkanAPI::lvkQueueSubmit(graphics_queue, 1, &submit_info, context_->in_flight_fences[context_->current_frame]));

    VkSwapchainKHR swapchains[] = {context_->swapchain->get_handle()};
    ...
    LICHT_VULKAN_CHECK(VulkanAPI::lvkQueuePresentKHR(present_queue, &present_info));
