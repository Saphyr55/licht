...
void VulkanGraphicsPipeline::init(VulkanContext* p_context) {
    ...
    LICHT_VULKAN_CHECK(p_context->rhi.licht_vkCreatePipelineLayout(p_context->device, &pipeline_layout_create_info, p_context->allocator, &pipeline_layout_));
    ...
    LICHT_VULKAN_CHECK(p_context->rhi.licht_vkCreateGraphicsPipelines(p_context->device, VK_NULL_HANDLE, 1, &graphics_pipeline_create_info, p_context->allocator, &graphics_pipeline_));
...
void VulkanGraphicsPipeline::destroy(VulkanContext* p_context) {
    ...
    p_context->rhi.licht_vkDestroyPipeline(p_context->device, graphics_pipeline_, p_context->allocator);
    p_context->rhi.licht_vkDestroyPipelineLayout(p_context->device, pipeline_layout_, p_context->allocator);
}

void vulkan_pipeline_bind(VulkanContext* p_context, VkCommandBuffer p_command_buffer) {
    p_context->rhi.licht_vkCmdBindPipeline(p_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, p_context->graphics_pipeline->pipeline());
}
