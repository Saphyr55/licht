...
bool physical_device_check_extension_support(VulkanContext* p_context,
    ...
    LICHT_VULKAN_CHECK(p_context->api.licht_vkEnumerateDeviceExtensionProperties(p_physical_device->handle, nullptr, &extension_count, nullptr));

    Array<VkExtensionProperties> available_extensions(extension_count);
    LICHT_VULKAN_CHECK(p_context->api.licht_vkEnumerateDeviceExtensionProperties(p_physical_device->handle, nullptr, &extension_count, available_extensions.data()));
...
bool physical_device_query_device_properties(VulkanContext* p_context, VulkanPhysicalDevice* p_physical_device) {
    ...
    p_context->api.licht_vkGetPhysicalDeviceProperties(p_physical_device->handle, &p_physical_device->properties);
...
bool physical_device_query_device_features(VulkanContext* p_context, VulkanPhysicalDevice* p_physical_device) {
    ...
    p_context->api.licht_vkGetPhysicalDeviceFeatures(p_physical_device->handle, &p_physical_device->features);
...
bool physical_device_query_queue_families(VulkanContext* p_context, VulkanPhysicalDevice* p_physical_device) {
    ...
    p_context->api.licht_vkGetPhysicalDeviceQueueFamilyProperties(p_physical_device->handle, &queue_family_count, nullptr);
    if (queue_family_count == 0) {
        return false;
    }

    p_physical_device->queue_families.resize(queue_family_count);
    p_context->api.licht_vkGetPhysicalDeviceQueueFamilyProperties(p_physical_device->handle, &queue_family_count, p_physical_device->queue_families.data());

    for (uint32 i = 0; i < queue_family_count; i++) {
        const VkQueueFamilyProperties& queue_family = p_physical_device->queue_families[i];

        VkBool32 is_present_support = false;
        LICHT_VULKAN_CHECK(p_context->api.licht_vkGetPhysicalDeviceSurfaceSupportKHR(p_physical_device->handle, i, p_context->surface, &is_present_support));
...
bool physical_device_select(VulkanContext* p_context, VulkanPhysicalDevice* p_physical_device) {
    ...
    LICHT_VULKAN_CHECK(p_context->api.licht_vkEnumeratePhysicalDevices(p_context->instance, &device_count, nullptr));
    if (device_count == 0) {
    ...
    LICHT_VULKAN_CHECK(p_context->api.licht_vkEnumeratePhysicalDevices(p_context->instance, &device_count, devices.data()));
