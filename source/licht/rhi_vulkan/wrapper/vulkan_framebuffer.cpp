...
void vulkan_framebuffers_init(VulkanContext* p_context) {
    ...
    for (usize i = 0; i < p_context->swapchain_image_views.size(); i++) {
        ...
        LICHT_VULKAN_CHECK(p_context->rhi.licht_vkCreateFramebuffer(p_context->device, &framebuffer_create_info, p_context->allocator, &p_context->framebuffers[i]));
    }
...
void vulkan_framebuffers_destroy(VulkanContext* p_context) {
    for (VkFramebuffer framebuffer : p_context->framebuffers) {
        p_context->rhi.licht_vkDestroyFramebuffer(p_context->device, framebuffer, p_context->allocator);
    }
}
