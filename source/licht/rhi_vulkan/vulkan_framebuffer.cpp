...
void vulkan_framebuffers_init(VulkanContext* p_context) {
    ...
    p_context->framebuffers.resize(p_context->swapchain->get_image_views().size());

    for (usize i = 0; i < p_context->swapchain->get_image_views().size(); i++) {
        VkImageView attachments[] = {p_context->swapchain->get_image_views()[i]};
        ...
        framebuffer_create_info.width = p_context->swapchain->get_extent().width;
        framebuffer_create_info.height = p_context->swapchain->get_extent().height;
...
void vulkan_framebuffers_destroy(SharedRef<VulkanDevice> p_device, Array<VkFramebuffer> p_framebuffers, VkAllocationCallbacks* p_allocator) {
