...
concept CAllocator = requires(AllocatorType allocator, ResourceType* resource, usize n) {
    { allocator.allocate(n) } -> std::same_as<ResourceType*>;
    { allocator.deallocate(resource, n) } -> std::same_as<void>;
};

template <typename AllocatorType, typename ResourceType>
concept CAlignedAllocator = requires(AllocatorType allocator, ResourceType* resource, usize n, usize alignment) {
