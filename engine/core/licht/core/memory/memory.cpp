...
uint8* Memory::allocate(size_t size, MemoryCategory category) noexcept {
    return new (category) uint8[size];
}

uint8* Memory::allocate(size_t size, size_t alignment, MemoryCategory category) noexcept {
    LCHECK_MSG(alignment > 0, "Alignment must be greater than zero.");
    LCHECK_MSG((alignment & (alignment - 1)) == 0, "Alignment must be a power of two.");

    // To ensure we can align the memory, we allocate extra bytes.
    size_t total_size = size + alignment;
    uint8* raw_memory = new (category) uint8[total_size];
...
void* operator new(size_t size, licht::MemoryCategory category) noexcept {
...
void operator delete(void* resource, size_t size, licht::MemoryCategory category) noexcept {
