...
void RHIVulkanCommandBuffer::bind_pipeline(RHIPipeline* pipeline) {
    RHIVulkanPipeline* vulkan_graphics_pipeline = static_cast<RHIVulkanPipeline*>(pipeline);
    VulkanAPI::lvkCmdBindPipeline(command_buffer_, VK_PIPELINE_BIND_POINT_GRAPHICS, vulkan_graphics_pipeline->get_handle());
}

void RHIVulkanCommandBuffer::bind_descriptor_sets(RHIPipeline* pipeline, const Array<RHIDescriptorSet*>& descriptor_sets) {
    static constexpr auto mapping_desc_set = [](RHIDescriptorSet*& descriptor_set) -> VkDescriptorSet {
        RHIVulkanDescriptorSet* vk_descriptor_set = static_cast<RHIVulkanDescriptorSet*>(descriptor_set);
    ...
    RHIVulkanPipeline* vk_pipeline = static_cast<RHIVulkanPipeline*>(pipeline);
...
void RHIVulkanCommandBuffer::bind_vertex_buffers(const Array<RHIBuffer*>& buffers) {
    ...
    Array<VkBuffer> vk_buffers = buffers.map<VkBuffer>([](RHIBuffer* buffer_handle) -> VkBuffer {
        return static_cast<RHIVulkanBuffer*>(buffer_handle)->get_handle();
    });

    VulkanAPI::lvkCmdBindVertexBuffers(command_buffer_, 0, buffers.size(), vk_buffers.data(), offsets.data());
}

void RHIVulkanCommandBuffer::bind_index_buffer(RHIBuffer* buffer) {
    VkBuffer vkbuffer = static_cast<RHIVulkanBuffer*>(buffer)->get_handle();
...
void RHIVulkanCommandBuffer::copy_buffer(RHIBuffer* source, RHIBuffer* destination, const RHIBufferCopyCommand& command) {
    RHIVulkanBuffer* vksource = static_cast<RHIVulkanBuffer*>(source);
    RHIVulkanBuffer* vkdestination = static_cast<RHIVulkanBuffer*>(destination);
...
void RHIVulkanCommandBuffer::begin_render_pass(const RHIRenderPassBeginInfo& begin_info) {
    RHIVulkanRenderPass* vk_render_pass = static_cast<RHIVulkanRenderPass*>(begin_info.render_pass);
    RHIVulkanFramebuffer* vk_framebuffer = static_cast<RHIVulkanFramebuffer*>(begin_info.framebuffer);
...
void RHIVulkanCommandAllocator::reset_command_buffer(RHICommandBuffer* command_buffer) {
    RHIVulkanCommandBuffer* vk_command_buffer = static_cast<RHIVulkanCommandBuffer*>(command_buffer);
...
void RHIVulkanCommandAllocator::allocate_command_buffers() {
    ...
    for (size_t i = 0; i < count_; i++) {
        upper_command_buffers_[i] = lnew(upper_command_buffer_allocator_, RHIVulkanCommandBuffer(command_buffers_[i]));
    }
}
...
RHIVulkanCommandAllocator::RHIVulkanCommandAllocator(VulkanContext& context, const RHICommandAllocatorDescription& description)
    ...
    upper_command_buffer_allocator_.initialize(count_ * sizeof(RHIVulkanCommandBuffer));
